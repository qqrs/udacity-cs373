...
def plan(road, lane_change_cost, init, goal): # Don't change the name of this function!
    ...
    max_speed = max(reduce(max, road))
    #heuristic = [[(goal[0]-i + goal[1]-j)*(1.0/road[i][j])
    #heuristic = [[(goal[0]-i + goal[1]-j)*(1.0/max_speed)
    heuristic = [[(goal[1]-j)*(1.0/max_speed) + abs(goal[0]-i)*lane_change_cost
    #heuristic = [[0
        for j in range(len(road[0]))] for i in range(len(road))]
    moves = [[0,1], [1,1], [-1,1]]
    move_costs = [0, lane_change_cost, lane_change_cost]
    ...
    costs = [[-1 for row in range(len(road[0]))] for col in range(len(road))]
    ...
    while not found and not resign:
        if len(open) == 0:
            ...
            open.sort(key=lambda op: (op[0] + heuristic[op[1]][op[2]], op[0]))
            #open.sort(key=lambda op: (op[0] + heuristic[op[1]][op[2]], op[0]),
                                                                #reverse=True)
            #open.reverse()
            #next = open.pop()
            ...
            if x == goal[0] and y == goal[1]:
                ...
                for i in range(len(moves)):
                    ...
                    if x2 >= 0 and x2 < len(road) and y2 >=0 and y2 < len(road[0]):
                        ...
                        g2 = g + 1.0/road[x][y] + move_costs[i]
                        #print "[x2,y2,cost]=%d, %d, %f" % (x2,y2,g2)
                        if closed[x2][y2] != 0:
                            for j in range(len(open)):
                                if open[j][1] == x2 and open[j][2] == y2:
                                    if open[j][0] > g2:
                                        open.pop(j)
                                        print "removing open[%d]" % j
                                    break
...
#solution_check([[test_road1],[lane_change_cost1],[test_init1],[test_goal1],[true_cost1]])
#solution_check([[test_road2],[lane_change_cost2],[test_init2],[test_goal2],[true_cost2]])
