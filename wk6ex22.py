...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    def slam_update(omega,xi,n0,n1,delta_pos,strength):
        for axis in range(2):
            a = 2*n0 + axis
            b = 2*n1 + axis
            omega.value[a][a] += strength
            omega.value[b][b] += strength
            omega.value[a][b] += -strength
            omega.value[b][a] += -strength
            xi.value[a][0] += -(delta_pos[axis] * strength)
            xi.value[b][0] += delta_pos[axis] * strength
        return (omega,xi)

    for i in range(len(data)):
        Z = data[i][0]
        (omega,xi) = slam_update(omega,xi,i,i+1,data[i][1],1.0/motion_noise)
        for j in range(len(Z)):
            (omega,xi) = slam_update(omega,xi,i,landmark_start+Z[j][0],
                                    [Z[j][1],Z[j][2]], 1.0/measurement_noise)
