...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        #n = k * 2 
        #dim += 2
        Omega = Omega.expand(dim,dim, [0,1] + range(4,dim))
        Xi = Xi.expand(dim,1, [0,1] + range(4,dim), [0])
        #dimmap = range(n+2)+range(n+4,dim)
        #Omega = Omega.expand(dim,dim, dimmap)
        #Xi = Xi.expand(dim,1, dimmap, [0])
        ...
        # reduce Omega and Xi
        A = Omega.take(range(2), range(2,dim))
        B = Omega.take(range(2), range(2))
        C = Xi.take(range(2), [0])
        Omega_prime = Omega.take(range(2,dim), range(2,dim))
        Xi_prime = Xi.take(range(2,dim),[0])
        #Omega.show()
        #A.show()
        #B.show()
        #Omega_prime.show()
